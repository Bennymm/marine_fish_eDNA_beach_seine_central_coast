...
#read data ####
asv_matrix <- read.table("no_occupancy_model/Data/2022_10_31/sequence_table.12S.merged.w_ASV_names.length_var.merged_dataset.txt",
                         header = T)

sample_data <- read_csv("no_occupancy_model/Data/2022_10_31/Calvert_12S_metadata.csv")
taxa <- read.delim("no_occupancy_model/Data/2022_10_31/taxonomy_table.12S.NCBI_NT.96sim.txt",
                   h=TRUE,
                   fill = TRUE)
...
s1 <- ASVmatrix %>%
 # rownames_to_column(., var = "surveyID") %>%
  pivot_longer(., !surveyID, names_to = "ASV", values_to = "count")

s2 <- merge(site_data, s1, by = "surveyID") %>%
...
saveRDS(eDNA_long, "no_occupancy_model/Data/2022_10_31/eDNA_long.rds")

#save new RDS files ####
saveRDS(site_data, file = "no_occupancy_model/Data/2022_10_31/derived_data/sitesurvey_data.rds")
saveRDS(sample_data, file = "no_occupancy_model/Data/2022_10_31/derived_data/sample_data.rds")
saveRDS(ASVmatrix, file = "no_occupancy_model/Data/2022_10_31/derived_data/ASVmatrix.rds")
