...
bs_long1 <- read_rds("no_occupancy_model/Data/2022_10_31/bs_long.rds")
eDNA_long1 <- read_rds("no_occupancy_model/Data/2022_10_31/eDNA_long.rds")
bs_taxonomy <- read_rds("no_occupancy_model/Data/2022_10_31/derived_data/BS_taxonomy_reconciled.rds")
eDNA_taxonomy <- read_rds("no_occupancy_model/Data/2022_10_31/derived_data/ASV_taxonomy_reconciled.rds")
taxonomy_shared <- read_rds("no_occupancy_model/Data/2022_10_31/derived_data/taxonomy_combined.rds")
...
write_rds(long, "no_occupancy_model/Data/2022_10_31/shared_long.rds")
...
write.csv(b4,"no_occupancy_model/Figures_tables/LCT_table.csv")
...
ggsave("no_occupancy_model/Figures_tables/biomass_x_read_facet_spec.png", 
...
write_csv(m6, "no_occupancy_model/Figures_tables/BM_read_regressions.csv")

#mean and median of R2 vales for biomass density against read index
mean(m6$R2) 
median(m6$R2)
...
ggplot(w8, aes(x = LCT_shared, y = weight1, colour = group1)) +
  ...
  scale_color_manual(name = "Observation type", 
                     labels = c("detected with both methods (n = 484)",
                                "not detected with either method (n = 2532)",
                                "beach seine detection only (n = 135)",
                                "eDNA detection only (n = 1187)"),
                     values=c("detected with both methods" = "#000000", 
                              "beach seine detection only" = "#E69F00",
                              "eDNA detection only" = "#D55E00",
...
ggsave("no_occupancy_model/Figures_tables/biomass_x_read_all.png", 
...
biomassDens_x_read_all <-
  ...
  scale_color_manual(name = "Observation type", 
                     labels = c( "beach seine detection only (n = 135)",
                                 "detected with both methods (n = 484)",
                                "eDNA detection only (n = 1187)",
                                "not detected with either method (n = 2932)"),
...
missed_obs_ebe
ggsave("no_occupancy_model/Figures_tables/missed_obs_ebe.png", 
       plot = missed_obs_ebe,
...
write_rds(x1, "no_occupancy_model/Data/2022_10_31/percent_missed.rds")
...
p_silt <- ggplot() +
  ...
  annotate("pointrange", x= 4,xmin = 3.5, xmax = 4.5, y=48, linewidth = 1, colour = "#D55E00") +
