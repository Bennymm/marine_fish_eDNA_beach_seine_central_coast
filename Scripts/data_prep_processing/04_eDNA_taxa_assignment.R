...
ASVbysite <- readRDS("Data/2022_10_31/derived_data/data12se_asvmatrix_lor_12s_ei.rds")
...
#check accepted naming and get higher taxonomy - turn on below to run process (select first option for all multiple hits)
...
#  cbind(.) %>%
#  rename("ids" = "query")
#write_rds(top10_gbifid_higher, "Data/2022_10_31/derived_data/top10_gbifid_higher.rds")
top10_gbifid_higher <- read_rds("Data/2022_10_31/derived_data/top10_gbifid_higher.rds")
...
##################################manual editing required on next lines###########################################
#write_csv(new_taxonomy, "Data/2022_10_31/derived_data/taxonomyt2.csv")
...
#rename as below
new_taxa <- read_csv("Data/2022_10_31/derived_data/taxonomy_range_annotated.csv") #import annotated taxa
...
##################################manual editing required on next lines########################################### 
q6$LCT <- c("Anoplarchus1", "Pholis1", "Hexagrammos1", "Sebastes1", "Xiphister1",         ######add a group name (in order) for each row in q6#######
             "Oligocottus1", "Myoxocephalus1", "Cottus1",  "Lepidopsetta1")
...
table <- data[c("level", "LCT", "class", "order", "family", "genus", "species", "all_species")] %>%
  distinct()
write_csv(table, "Data/2022_10_31/derived_data/taxonomy_groups_12s_eDNA20221103.csv")

write_rds(data, "Data/2022_10_31/derived_data/ASV_taxonomy_eDNA20221103.rds")
...
write_rds(LCT_by_site, "Data/2022_10_31/derived_data/LCT_by_site.rds")

#make table for augmenting Beach Seine taxonomy

b1 <- data
b2 <- b1 %>% separate(all_species, c("A", "B", "C", "D"), sep = ", ")
...
write_rds(b3, "Data/2022_10_31/derived_data/taxonomy_eDNA.rds")
...
length(unique(top10_occ$ASV)) #fish reads passing occupancy model
length(unique(top10_occ_notfish$ASV)) #non-target reads passing occupancy model
sum(LCT_by_site) #number of fish reads after Occupancy modelling
sum(ASVbysite) #total number of reads passing occupancy modelling
