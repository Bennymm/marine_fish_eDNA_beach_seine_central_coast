...
library(lubridate)
#read in raw data
bs <- read_csv("Data/2022_10_31/bs_data/bs_abundance.csv")
taxa_BS <- readRDS("Data/2022_10_31/derived_data/taxa_BS_resolved.rds")
env <- read_csv("Data/2022_10_31/bs_data/bs_habitat.csv")
sitesurvey_data_eDNA <- readRDS("Data/2022_10_31/derived_data/sitesurvey_data.rds")
taxa_eDNA <- read_rds("Data/2022_10_31/derived_data/taxonomy_eDNA.rds")
...
write_rds(BS_taxonomy, "Data/2022_10_31/derived_data/taxonomy_BS.rds")
...
write_rds(BS_taxonomy_reconciled, "Data/2022_10_31/derived_data/BS_taxonomy_reconciled.rds")

ASV_taxonomy <- read_rds("Data/2022_10_31/derived_data/ASV_taxonomy_eDNA20221103.rds") 
...
write_rds(ASV_taxonomy_reconciled, "Data/2022_10_31/derived_data/ASV_taxonomy_reconciled.rds")


#create lca.taxon variable
#taxa$lca_taxon <- ifelse(is.na(taxa$order), taxa$class,
#                         ifelse(is.na(taxa$family), taxa$order,
#                                ifelse(is.na(taxa$genus), taxa$family,
#                                       ifelse(is.na(taxa$species), taxa$genus, taxa$species))))

#create a unique number for duplicate genera - we know that the Sebastes and Onchorhynchus here are different species
#taxa$unique_taxa <- make.unique(as.character(taxa$lca_taxon), sep = "") 

#write.csv(taxa, 'Data/2021_09_20/derived_data/taxaforQC.csv')
#saveRDS(taxa, "Data/2021_09_20/derived_data/taxa_BS.rds")
...
saveRDS(t2, "Data/2022_10_31/derived_data/bs_specmatrix_retainreplicates.rds") # maybe we do some occupancy modelling later
...
saveRDS(t5, "Data/2022_10_31/derived_data/bs_specmatrix.rds")

t6 <- t4[c("site", "year", "month", "day", "surveyID")] # select survey data
saveRDS(t6, "Data/2022_10_31/derived_data/bs_surveydata.rds") #could add environmental to this at some point
...
write_rds(s4, "Data/2022_10_31/derived_data/taxonomy_combined.rds")
